...
6. Write output WAV at original input sample rate and channel count
...
The primary output. WAV at original input sample rate and channel count.
...
└── <source_stem>/
    ...
    └── output/
        ├── <source_stem>_<template>_<duration>_v<N>.wav
...
uv run jam2song "$src" --structure loop_build_drop  -o "$out/01-260320_2143_loop_build_drop_3m30s_v7.wav"
uv run jam2song "$src" --structure verse_chorus     -o "$out/01-260320_2143_verse_chorus_3m30s_v7.wav"
uv run jam2song "$src" --structure highlight_reel   -o "$out/01-260320_2143_highlight_reel_3m30s_v7.wav"
```
